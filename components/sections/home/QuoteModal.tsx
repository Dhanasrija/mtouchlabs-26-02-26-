    ...
    <>
      ...
      <script dangerouslySetInnerHTML={{ __html: `
        (function(){
          document.addEventListener('click', function(e){
            var ov = document.getElementById('requestQuoteModal');
            if(!ov) return;
            var t = e.target;
            while(t && t !== document){
              // CLOSE: clicked the X button or anything inside it
              if(t.id === 'rqCloseBtn' || (t.classList && t.classList.contains('rq-close-btn'))){
                e.preventDefault();
                e.stopPropagation();
                ...
                var w = document.getElementById('rqFormWrap');
                if(s) s.style.display = 'none';
                if(w) w.style.display = '';
                return;
              }
              // OPEN: clicked any js-open-modal trigger
              if(t.classList && (t.classList.contains('js-open-modal') || (t.dataset && t.dataset.openQuote === '1'))){
                e.preventDefault();
                e.stopPropagation();
      ...
      <script dangerouslySetInnerHTML={{ __html: `
        (function(){
          ...
          /* close-button + overlay-backdrop close are handled by the delegation script above */
