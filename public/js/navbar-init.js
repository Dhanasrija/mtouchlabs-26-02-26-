...
(function () {
  ...
  // ─── MASTER PATH→SECTION MAP ───────────────────────────────────────────────
  // This is the single source of truth for top-level nav highlighting.
  // Must stay in sync with Navbar.tsx sets (SERVICE_PATHS, PRODUCT_PATHS, etc.)
  // ───────────────────────────────────────────────────────────────────────────
  var SECTION_MAP = {
    // HOME
    home: [
      ...
      '/vision-mission', '/leadership-team', '/our-journey',
      '/awards-recognition', '/nasscom-membership', '/life-at-mtouch',
      '/about', '/about-us', '/blog', '/blogs', '/case-studies',
      '/our-vision', '/clutch',
    ],

    // SERVICES — every explicit service slug
    services: [
      '/services',
      '/mobile-app-development-company', '/ios-app-development-company',
      '/android-app-development-company', '/ipad-app-development-company',
    ...
    // PRODUCTS — every explicit product slug
    // *** ecommerce-app-development-company is HERE, NOT in services ***
    products: [
      '/on-demand-products-development-company-hyderabad',
      '/ecommerce-app-development-company',          // ← KEY FIX
    ...
    // PORTFOLIO — exact page + all detail pages
    portfolio: ['/portfolio'],
  ...
  // Nav link selectors for each section
  var NAV_SELECTORS = {
    home:      '.menu-links[href="/"]',
    services:  '.menu-links[href="/services"]',
    products:  '.menu-links[href="/on-demand-products-development-company-hyderabad"]',
    resources: '.menu-links[href="/hire-developers-in-india"]',
    portfolio: '.menu-links[href="/portfolio"]',
    careers:   '.menu-links[href="/careers"]',
    contact:   '.menu-links[href="/contact-us"]',
  };

  // function getSection(pathname) {
  //   // Normalise: lowercase, strip trailing slash, strip query/hash
  //   var p = pathname.split('?')[0].split('#')[0].toLowerCase();
  //   if (p.length > 1 && p.endsWith('/')) p = p.slice(0, -1);
  //   if (!p.startsWith('/')) p = '/' + p;

  //   // 1. Exact match first (highest priority — prevents pattern false-positives)
  //   for (var section in SECTION_MAP) {
  //     if (SECTION_MAP[section].indexOf(p) !== -1) return section;
  //   }

  //   // 2. Prefix matches for dynamic sub-routes
  //   if (p.startsWith('/portfolio/')) return 'portfolio';
  //   if (p.startsWith('/blog/') || p.startsWith('/blogs/')) return 'home';
  //   if (p.startsWith('/case-studies/')) return 'home';
  //   if (p.startsWith('/about/') || p.startsWith('/about-us/')) return 'home';
  //   if (p.startsWith('/careers/')) return 'careers';
  //   if (p.startsWith('/contact-us/')) return 'contact';
  //   if (p.startsWith('/hire-')) return 'resources';

  //   // 3. Pattern fallback for service slugs NOT explicitly listed
  //   //    Only fires if none of the above matched — portfolio/product paths
  //   //    are already caught above so no false-positives.
  //   if (
  //     p.endsWith('-services') ||
  ...
  //   return null; // unknown page — don't change anything
  // }
  ...
  // ✅ ALWAYS handle portfolio FIRST (this fixes your bug)
  if (p === '/portfolio' || p.startsWith('/portfolio/')) {
    return 'portfolio';
  }
  ...
  // exact match
  for (var section in SECTION_MAP) {
    if (SECTION_MAP[section].indexOf(p) !== -1) {
      return section;
    }
  }
  ...
  // blog / about
  if (p.startsWith('/blog') || p.startsWith('/blogs')) return 'home';
  if (p.startsWith('/about')) return 'home';
  if (p.startsWith('/case-studies')) return 'home';

  // ✅ service pattern (LAST)
  if (
    p.endsWith('-services') ||
    p.endsWith('-solutions') ||
    p.endsWith('-company') ||
  ...
  function setActiveNavLink() {
    ...
    var section = getSection(pathname);

    // Remove active-link from ALL top-level nav links first
    document.querySelectorAll('.ree-nav .menu-links').forEach(function (link) {
      link.classList.remove('active-link');
    });

    // Apply active-link to the correct section link
    if (section && NAV_SELECTORS[section]) {
      var activeLink = document.querySelector(NAV_SELECTORS[section]);
      if (activeLink) {
        activeLink.classList.add('active-link');
      }
    }

    // Also highlight the active dropdown sub-link
    setActiveDropdownLink(pathname);
  }

  function setActiveDropdownLink(currentPath) {
    // Clean up any previous dropdown active states
    document.querySelectorAll('.menu-dropdown a.page-active, .quick-links a.page-active').forEach(function (link) {
      link.classList.remove('page-active');
      link.style.color = '';
      link.style.fontWeight = '';
    ...
    var activeDropdownLink = null;
    for (var i = 0; i < possibleSelectors.length; i++) {
      activeDropdownLink = document.querySelector(
        '.menu-dropdown ' + possibleSelectors[i] + ', .quick-links ' + possibleSelectors[i]
      );
      if (activeDropdownLink) break;
    }
  ...
  function initNavbar() {
    ...
    function cancelHideTimer() {
      if (globalHideTimer) { clearTimeout(globalHideTimer); globalHideTimer = null; }
    }
    ...
    megaMenus.forEach(function (menu) {
      var dropdown = menu.querySelector('.menu-dropdown');
      if (!dropdown) return;
      menu.addEventListener('mouseenter', function () { cancelHideTimer(); showMenuDropdown(menu, dropdown); });
      menu.addEventListener('mouseleave', function () { startHideTimer(); });
    });

    dropdowns.forEach(function (dd) {
      dd.addEventListener('mouseenter', function () { cancelHideTimer(); });
      dd.addEventListener('mouseleave', function () { startHideTimer(); });
    });
    ...
    if (menuHeader) {
      menuHeader.addEventListener('mouseleave', function (e) {
        var related = e.relatedTarget;
        if (related && (related.closest('.menu-dropdown') || related.closest('.megamenu'))) return;
        startHideTimer();
      });
    }
    // ═══ END GLOBAL HOVER SYSTEM ═══════════════════════════════════════════

    // Panel switching for mega-menu sidebar items (js-home-mega-item)
    document.querySelectorAll('.home-mega-sidebar').forEach(function (sidebar) {
      var items = sidebar.querySelectorAll('.js-home-mega-item');
      var megaContent = sidebar.closest('.home-mega-layout') && sidebar.closest('.home-mega-layout').querySelector('.home-mega-content');
      ...
      items.forEach(function (item) {
        item.addEventListener('mouseenter', function () {
          ...
          megaContent.querySelectorAll('.home-mega-panel').forEach(function (panel) {
            panel.classList.remove('active');
          });
          var panel = megaContent.querySelector('#' + target);
          if (panel) panel.classList.add('active');
        });
      });
    });

    // Panel switching for old home-mega dropdown (data-service / data-content)
    document.querySelectorAll('.menu-dropdown.home-mega').forEach(function (dropdown) {
      var quickLinks = dropdown.querySelectorAll('.quick-links a[data-service]');
      if (!quickLinks || quickLinks.length === 0) return;
      ...
      function showPanelForService(svc) {
        var key = svc ? svc.replace(/^custom-/, '') : '';
        dropdown.querySelectorAll('[data-content]').forEach(function (p) { p.classList.remove('active'); });
        dropdown.querySelectorAll('.quick-links a').forEach(function (l) { l.classList.remove('is-current'); });
        var target = dropdown.querySelector('[data-content="' + svc + '"]') ||
                     dropdown.querySelector('[data-content="' + key + '"]') ||
                     dropdown.querySelector('[data-content="custom-' + key + '"]');
        var activeLink = dropdown.querySelector('.quick-links a[data-service="' + svc + '"]');
        if (activeLink) activeLink.classList.add('is-current');
      ...
      quickLinks.forEach(function (link) {
        link.addEventListener('mouseenter', function () { showPanelForService(this.getAttribute('data-service') || ''); });
        link.addEventListener('click', function () { showPanelForService(this.getAttribute('data-service') || ''); });
      });
    });
