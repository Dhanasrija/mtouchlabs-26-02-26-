...
//
// CATEGORY MODEL (6 fixed, balanced categories)
//   1. eCommerce & Retail                  — 11
//   2. Food, Grocery & Delivery            — 11
//   3. Healthcare & Education              — 10
//   4. Real Estate & Mobility              — 10
//   5. Enterprise, CRM & Service Platforms — 10
//   6. Social, Media & Lifestyle           — 10
//   ───────────────────────────────────────────
//   Total 62 projects — every project belongs to exactly one category.
//   The `category` value MUST match a data-filter value in
//   app/portfolio/page.tsx exactly (compared case-insensitively).
// ─────────────────────────────────────────────────────────────────
window.__mtlInitPortfolioList = function initPortfolio() {
  ...
  const blogCards = [
    ...
    },
    {
      ...
      tag: "UX, UI, Android",
      category: "Enterprise, CRM & Service Platforms"
    },
    {
  ...
  // Expose the dataset so other scripts / debugging can inspect it.
  window.__mtlPortfolioCards = blogCards;
  ...
  function renderCards(page) {
    ...
    pageItems.forEach(function (item) {
      ...
      var href = (/^https?:\/\//i.test(rawLink) || rawLink.charAt(0) === '/')
        ? rawLink
        : '/portfolio/' + rawLink;
      var cat = normalizeCategory(item);
      var cardHTML = ''
        + '<article class="col-lg-4 col-sm-6 blog-card-item pf-card-col" data-category="' + escapeHtml(cat) + '">'
        +   '<div class="ree-media-crd pf-card">'
        +     '<div class="rpl-img pf-card-media">'
        +       '<a href="' + escapeHtml(href) + '" target="_blank" rel="noopener noreferrer" tabindex="-1" aria-hidden="true">'
        +         '<img src="' + escapeHtml(item.image) + '" alt="' + escapeHtml(item.name) + '" class="fill-fixed" loading="lazy" decoding="async" width="400" height="260">'
        +       '</a>'
        +       '<span class="pf-card-cat">' + escapeHtml(cat) + '</span>'
        +     '</div>'
        +     '<div class="rpl-contt pf-card-body">'
        +       '<p class="port-tags pf-card-tags">' + escapeHtml(item.tag) + '</p>'
        +       '<h3 class="pf-card-title"><a href="' + escapeHtml(href) + '" target="_blank" rel="noopener noreferrer">' + escapeHtml(item.name) + '</a></h3>'
        +       '<a href="' + escapeHtml(href) + '" target="_blank" rel="noopener noreferrer" class="blog-read-more pf-card-link" aria-label="Read more about ' + escapeHtml(item.name) + '">Read More <i class="fas fa-arrow-right" aria-hidden="true"></i></a>'
        +     '</div>'
        +   '</div>'
        + '</article>';
    ...
    // Re-init the reveal animation for the newly added cards.
    setTimeout(function () {
      var cardsEls = blogContainer.querySelectorAll('.ree-media-crd');
      cardsEls.forEach(function (card, index) {
        card.classList.remove('animate-in');
        var row = Math.floor(index / 3);
        var colIndex = index % 3;
        var delay = 60 + row * 90 + colIndex * 50;
        setTimeout(function () { card.classList.add('animate-in'); }, delay);
      });
    }, 30);
  }
  ...
  function renderPagination(totalPages) {
    ...
    nav.setAttribute('aria-label', 'Portfolio pagination');
    ...
    function createPageItem(pageNum, label, isActive) {
      ...
      btn.type = 'button';
      btn.className = 'pf-page-btn' + (isActive ? ' is-active' : '');
      btn.textContent = String(label || pageNum);
      if (isActive) btn.setAttribute('aria-current', 'page');
      btn.addEventListener('click', function (e) {
        e.preventDefault();
        renderCards(pageNum);
        var anchor = document.getElementById('portfolio-grid');
    ...
    if (currentPage > 1) ul.appendChild(createPageItem(currentPage - 1, 'Prev', false));
    ...
    if (currentPage < totalPages) ul.appendChild(createPageItem(currentPage + 1, 'Next', false));

    nav.appendChild(ul);
    paginationContainer.appendChild(nav);
  }

  // Wire up filter buttons using event delegation — robust against Next.js
  // hydration, late DOM availability, and re-renders. Bound once per page
  // load; it calls into the *current* invocation via the global below.
  if (!window.__mtl_portfolio_filter_bound) {
    window.__mtl_portfolio_filter_bound = true;
    document.addEventListener('click', function (e) {
      var btn = e.target && e.target.closest && e.target.closest('.blog-filter-btn');
      if (!btn) return;
      var filterValue = btn.getAttribute('data-filter') || 'all';
      window.__mtl_portfolio_filter = filterValue;
      document.querySelectorAll('.blog-filter-btn').forEach(function (b) {
        var on = b === btn;
        b.classList.toggle('active', on);
        b.setAttribute('aria-pressed', on ? 'true' : 'false');
      });
      if (typeof window.__mtlPortfolioApplyFilter === 'function') {
        window.__mtlPortfolioApplyFilter(filterValue);
      }
    });
  }

  window.__mtlPortfolioApplyFilter = function (filterValue) {
    currentFilter = filterValue || 'all';
    renderCards(1);
  };

  // Restore the previously selected filter after a client-side navigation.
  currentFilter = window.__mtl_portfolio_filter || 'all';
  document.querySelectorAll('.blog-filter-btn').forEach(function (b) {
    var isActive = (b.getAttribute('data-filter') || 'all').toLowerCase() === currentFilter.toLowerCase();
