...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  // Fallback date for DB rows that have no timestamp.
  const today = new Date().toISOString().split('T')[0]
  // Static pages: use a REAL last-content-change date, not `today`.
  // Stamping every page with the deploy date teaches Google to ignore lastmod.
  // ⚠️ Bump this ONLY when you actually change static page content.
  const staticLastModified = '2026-07-12'
  ...
  const servicePages = [
    ...
    // Digital Marketing (duplicates consolidated via 301 in next.config.js)
...
try {
  ...
  portfolios.forEach((portfolio: any) => {
    entries.push({
      // Canonical home is /portfolio/<slug> (middleware 301s the bare /<slug>
      // to it, and the page's canonical tag points there). The sitemap must
      // list final URLs, never URLs that redirect.
